...
const getDefaultBaseFormData = (): Omit<
  Recipe,
  | 'ingredients'
  | 'steps'
  | 'utensils'
...
const ingredientFactory = (): Omit<Ingredient, 'id'> => ({
  quantity: null,
  unit: null,
  name: '',
  notes: null,
...
const {
  ...
  itemFactory: ingredientFactory,
});

const stepFactory = (): Omit<Step, 'id'> => ({
  description: '',
  order: 1,
...
} = useFieldArray<Step>('steps', { itemFactory: stepFactory });

const utensilFactory = (): Omit<Utensil, 'id'> => ({ name: '' });
const {
  fields: utensilFields,
  append: appendUtensil,
  remove: removeUtensil,
  replace: replaceUtensils,
} = useFieldArray<Utensil>('utensils', {
  itemFactory: utensilFactory,
});
  ...
  (newInitialRecipe) => {
    if (newInitialRecipe) {
      ...
      const recipeToEdit = JSON.parse(
        JSON.stringify(newInitialRecipe)
      );
      ...
      replaceIngredients(
        recipeToEdit.ingredients &&
          ...
          ? recipeToEdit.ingredients.map(
              (ing: Partial<Ingredient>) => ({
                ...ingredientFactory(),
                ...ing,
              })
            ) // Add explicit type Partial<Ingredient>
...
const updateNullableStringField = (
  field: keyof typeof baseFormData,
  value: string
) => {
...
const updateIngredientField = (
  ...
  let processedValue: string | number | IngredientCategory | null =
    value;
  ...
  switch (field) {
    ...
    case 'unit':
    case 'notes':
    case 'category': // Add case for category (though not editable yet)
      processedValue =
        typeof value === 'string' && value.trim() !== ''
          ? value.trim()
          : null;
...
const handleSubmit = () => {
  const finalRecipe: Recipe = {
    ...baseFormData,
    id: props.initialRecipe?.id || null,
    ingredients: ingredientFields.value
      .map((ing: Ingredient) => ({
        ...ing,
        name: ing.name.trim(),
        category: ing.category,
      })) // Explicit Ingredient type
      .filter((ing) => ing.name !== ''),
    ...
    utensils: utensilFields.value
      .map((ut: Utensil) => ({ ...ut, name: ut.name.trim() })) // Explicit Utensil type
      .filter((ut) => ut.name !== ''),
    createdAt: props.initialRecipe?.createdAt ?? new Date(),
    updatedAt: new Date(),
      ...
      <div class="space-y-6">
        ...
        <UFieldset legend="Basic Information" class="space-y-4">
          ...
          <UFormGroup label="Description" name="description">
            <UTextarea
              ...
              @update:model-value="
                (value) =>
                  updateNullableStringField('description', value)
              "
              name="description"
          ...
          <div class="grid grid-cols-1 gap-4 sm:grid-cols-3">
            <UFormGroup label="Prep Time" name="prepTime">
              <UInput
                ...
                @update:model-value="
                  (value) =>
                    updateNullableStringField('prepTime', value)
                "
                name="prepTime"
            ...
            <UFormGroup label="Cook Time" name="cookTime">
              <UInput
                ...
                @update:model-value="
                  (value) =>
                    updateNullableStringField('cookTime', value)
                "
                name="cookTime"
            ...
            <UFormGroup label="Cuisine" name="cuisine">
              <UInput
                ...
                @update:model-value="
                  (value) =>
                    updateNullableStringField('cuisine', value)
                "
                name="cuisine"
          ...
          <UFormGroup label="Portions" name="portions" required>
            <UInput
              v-model.number="baseFormData.portions"
              type="number"
              name="portions"
              :min="1"
              required
        ...
        <UFieldset legend="Ingredients" class="space-y-4">
          <div
            ...
            :key="ingredient.id"
            class="ingredient-group grid grid-cols-1 sm:grid-cols-[1fr_1fr_2fr_auto] gap-x-2 gap-y-3 items-start border-b border-gray-200 dark:border-gray-700 pb-4 mb-4 last:border-b-0 last:pb-0 last:mb-0"
          >
            ...
            >
              <UInput
                ...
                @update:model-value="
                  (value) =>
                    updateIngredientField(index, 'quantity', value)
                "
                type="number"
                :name="`ingredient-quantity-${index}`"
                placeholder="e.g., 2"
            ...
            >
              <UInput
                ...
                @update:model-value="
                  (value) =>
                    updateIngredientField(index, 'unit', value)
                "
                :name="`ingredient-unit-${index}`"
            ...
            >
              <UInput
                ...
                @update:model-value="
                  (value) =>
                    updateIngredientField(index, 'name', value)
                "
                :name="`ingredient-name-${index}`"
            ...
            <UButton
              ...
              class="remove-item self-end mb-1 sm:col-start-4 sm:row-start-1"
            ...
            >
              <UInput
                ...
                @update:model-value="
                  (value) =>
                    updateIngredientField(index, 'notes', value)
                "
                :name="`ingredient-notes-${index}`"
            ...
            >
              <UInput
                :model-value="ingredient.category ?? 'N/A'"
                :name="`ingredient-category-${index}`"
                disabled
        ...
        <UFieldset legend="Steps" class="space-y-4">
          <div
            ...
            :key="step.id"
            class="step-group flex items-start gap-2 border-b border-gray-200 dark:border-gray-700 pb-4 mb-4 last:border-b-0 last:pb-0 last:mb-0"
          >
            <span
            ...
            >
              <UTextarea
                ...
                @update:model-value="
                  (value) => updateStepDescription(index, value)
                "
                :name="`step-description-${index}`"
        ...
        <UFieldset legend="Utensils" class="space-y-4">
          <div
            v-for="(utensil, index) in utensilFields"
            :key="utensil.id"
            class="utensil-group flex items-end gap-2 border-b border-gray-200 dark:border-gray-700 pb-4 mb-4 last:border-b-0 last:pb-0 last:mb-0"
          >
            <UFormGroup
              label="Utensil Name"
            ...
            >
              <UInput
                ...
                @update:model-value="
                  (value) => updateUtensilName(index, value)
                "
                :name="`utensil-name-${index}`"
